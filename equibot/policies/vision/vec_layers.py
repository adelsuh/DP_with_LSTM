...
class VecLSTM(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.scalar_out_flag = s_out > 0
        make_linear = lambda din, dout, sin, sout: VecLinear(
            din, dout,
            sin, sout,
            mode=mode, 
            vec2scalar_dir_learnable=vs_dir_learnable,
        ...
        make_act = lambda din, act_func: VecActivation(
            din, act_func,
            shared_nonlinearity, mode,
            cross=cross,
            safe_bound=safe_bound,
            act_mode=act_mode,
            pre_act_len_bound=pre_act_len_bound,
        ...
        self.wii = make_linear(v_in, self.hidden_size, s_in, 0)
        self.whi = make_linear(v_in, self.hidden_size, s_in, 0)
        self.whi = make_linear(v_in, self.hidden_size, s_in, 0)
        self.whi = make_linear(v_in, self.hidden_size, s_in, 0)
        self.whi = make_linear()
        self.whi = make_linear()
        self.whi = make_linear()
        self.whi = make_linear()
